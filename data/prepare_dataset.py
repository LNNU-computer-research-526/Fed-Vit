...
def prepare_polyp(save_dir, ori_dir):
    ...
    for i, site_name in enumerate(client_name):
        # 打印当前客户端及文件数量
        img_path = f'{ori_dir}/{site_name}/image/*'
        mask_path = f'{ori_dir}/{site_name}/mask/*'
        imgs = sorted(glob(img_path))
        masks = sorted(glob(mask_path))
        print(f"客户端{i + 1}（{site_name}）：图像数量={len(imgs)}，掩码数量={len(masks)}")  # 新增打印
        client_data_list.append(imgs)
        client_mask_list.append(masks)
    ...
    for client_idx in range(len(client_name)):
        ...
        for case_idx in range(len(seg_paths)):  # case
            ...
            for slice_idx in range(image.shape[0]):
                ...
                np.save('{}/client{}/data_npy/{}/{}.npy'.format(save_dir, client_idx + 1, case_name, slice_name),
                        sample)
...
prepare_polyp(save_dir='C:/Users/Administrator/Desktop/FedEvi-master/Dataset/Polyp_npy', ori_dir='C:/Users/Administrator/Desktop/FedEvi-master/Dataset/Polyp')
